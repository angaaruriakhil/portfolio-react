...
async function deriveKey(password) {
  ...
  return crypto.subtle.deriveKey(
    {
      name: "PBKDF2",
      salt: enc.encode("cv-salt"), // static is fine here (this is obfuscation)
      iterations: 1000,
      hash: "SHA-256",
    },
    keyMaterial,
...
async function aesEncrypt(plaintext, password) {
  ...
  // pack iv + ciphertext, then base64 it
...
async function locked_glink() {
  ...
  const cipher = await aesEncrypt(g_bare_message, secretKey2);
  return cipher; // base64 string, like CryptoJS .toString()
}

async function g_unlockedLink() {
  const secretKey2 = "secretKey2";
  const cipher = await locked_glink();
  const url = await aesDecrypt(cipher, secretKey2);
  return url;
}
// ---------------------------------------------------------------------
...
export default function Socials({ variant = "vertical" }) {
  const handleResumeClick = async (e) => {
    ...
    try {
      const url = await g_unlockedLink();
      if (newTab && url) {
        newTab.location = url;
      } else if (url) {
